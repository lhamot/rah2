        ...
        namespace niebloids
        {
            struct copy_fn
            {
                ...
                static constexpr RAH2_NS::ranges::copy_result<I, O> impl(I first, S last, O result)
                {
                    ...
                    auto const byte_size = (char*)&(*last2) - (char*)&(*first);
                    if (byte_size != 0)
                    {
                        memcpy((void*)&(*result), (void*)&(*first), byte_size);
                        auto const elt_count = (last - first);
                        first += elt_count;
                        result += elt_count;
                    }
                    return {RAH2_STD::move(first), RAH2_STD::move(result)};
                ...
                constexpr RAH2_NS::ranges::copy_result<I, O> operator()(I first, S last, O result) const
                {
                    ...
                    auto impl_result = impl(first2, last2, RAH2_STD::move(result));
                    return {
                        first_last.wrap_iterator(RAH2_STD::move(impl_result.in)),
                        RAH2_STD::move(impl_result.out)};
                }
...
#pragma GCC diagnostic ignored "-Wstringop-overflow"
            ...
            struct copy_backward_fn
            {
                template <
                    ...
                    RAH2_STD::enable_if_t<not(
                        ...
                        and RAH2_NS::contiguous_iterator<I1> and
                        RAH2_NS::contiguous_iterator<I2>
                        and RAH2_STD::is_trivially_copyable<RAH2_NS::iter_value_t<I1>>::value
                        and RAH2_NS::is_same_v<RAH2_NS::iter_value_t<I1>, RAH2_NS::iter_value_t<I2>>)>* =
                            nullptr
                    >
                ...
                constexpr copy_backward_result<I1, I2> operator()(I1 first, S1 last, I2 result) const
                {
                    ...
                    return {
                        ...
                        unwraped_out.wrap_iterator(RAH2_STD::move(ret.out))};
                }
        ...
        namespace niebloids
        {
            struct replace_fn
            {
                ...
                constexpr I operator()(
                    I first, S last, T1 const& old_value, T2 const& new_value, Proj proj = {}) const
                {
                    auto proj2 = RAH2_NS::ranges::details::move_unary(proj);
                    for (; first != last; ++first)
                        if (old_value == proj2(*first))
                            *first = new_value;
                    return first;
                }
        ...
        namespace niebloids
        {
            struct remove
            {
                ...
                template <typename ForwardIterator, typename ForwardSentinel, typename T, typename Proj>
                ...
                template <typename ForwardRange, typename T, typename Proj>
                borrowed_subrange_t<ForwardRange>
        ...
        namespace niebloids
        {
            struct replace_copy
            {
                    ...
                    Proj proj = {}) const
                {
                    ...
                    for (; first != last; ++first, ++result)
                    {
                        *result = (proj2(*first) == old_value) ? new_value : *first;
                    }
                    return {first, result};
                }
