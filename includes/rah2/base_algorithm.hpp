            ...
            struct rotate
            {
                ...
                operator()(ForwardIterator first, ForwardIterator middle, ForwardSentinel last) const
                {
                    if (middle != first)
                    {
                        ...
                        {
                            auto first_last =
                                details::unwrap(RAH2_STD::move(first), last);
                            auto middle_last =
                                details::unwrap(RAH2_STD::move(middle), RAH2_STD::move(last));

                            auto last2 = RAH2_NS::ranges::next(middle_last.iterator, middle_last.sentinel);
                            ...
                            using HelperType = details::rotate_helper<IC, is_trivially_move_assignable>;
